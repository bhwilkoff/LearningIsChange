...
save-contact (`meet/ben-wilkoff.vcf`), native share sheet, platform-aware
app store buttons, Universal App Template link, 30-second bio, and two
pre-rendered QR SVGs. `/meet/#show` and `/meet/#show-vcard` open a
full-screen QR for holding the phone up.

- QR codes + vCard are generated by `scripts/make-meet-qr.py` (needs
  `pip install segno`). Edit `CONTACT` there **and** the `phone`/email
  lines at the top of the inline script in `meet/index.html`, then re-run.
- Headshot: `meet/ben.jpg` (640px, page + blog sidebar widget) and
  `meet/ben-vcard.jpg` (240px, embedded as vCard PHOTO). Both cropped
  from the studio headshot with PIL; re-crop by hand if the photo changes.
- The blog sidebar fragment carries a "Did you just meet Ben?" widget
  linking to `/meet/` (regenerated site-wide 2026-09-16).
- The .vcf necessarily contains the email in plain text (the page itself
  assembles it in JS, like `support.html`).
